...
ui <- fluidPage(theme = shinytheme("united"),
                ...
                sidebarLayout(
                  sidebarPanel2(
                    ...
                    p("1=Game 1 \n 2=Game 2 \n 3=Game 3 \n 4=Game 4\n 5=All Games"),
                    out = p("The riddle: \n In a distant, war-torn land, there are 5 castles. There are 
                 ...
                 the castles are worth 1, 2, 3, 4, and 5 victory points respectively. 
                 You and your enemy each have 50 soldiers to distribute, any 
                 way you like, to fight at any of the 5 castles. Whoever sends
...
server <- function(input, output, session) {
  ...
  output$troopCommittedDisplay2 <- renderText({
    paste("Total Troops Committed: ", sum(troop()[6:10]))
  })
  
  output$troopCommittedAmount2 <- renderText({
    if(sum(troop()[6:10]) == 50) { 
      paste("At troop limit, press Submit to continue")
    } else if (sum(troop()[6:10]) > 50) {
      paste("Over troop limit!")
    } else if (sum(troop()[6:10]) < 50) {
      paste("Under troop limit, assign more troops")
    }
  ...
  output$distPlot <- renderPlot({
    if (submitClicked()) {
      castle_1_data <- dataToUse()[dataToUse()$Castle == "castle_1", ]
      castle_2_data <- dataToUse()[dataToUse()$Castle == "castle_2", ]
      castle_3_data <- dataToUse()[dataToUse()$Castle == "castle_3", ]
      castle_4_data <- dataToUse()[dataToUse()$Castle == "castle_4", ]
      castle_5_data <- dataToUse()[dataToUse()$Castle == "castle_5", ]
      ...
      p1 <- ggplot(castle_1_data, aes(x = Value, y = ..density..)) +
        ...
        geom_vline(aes(xintercept = troopValues$troop[6]), color = "blue", linewidth = 1.5) 
      ...
      p2 <- ggplot(castle_2_data, aes(x = Value, y = ..density..)) +
        ...
        geom_vline(aes(xintercept = troopValues$troop[7]), color = "blue", linewidth = 1.5)
      ...
      p3 <- ggplot(castle_3_data, aes(x = Value, y = ..density..)) +
        ...
        geom_vline(aes(xintercept = troopValues$troop[8]), color = "blue", linewidth = 1.5)
      ...
      p4 <- ggplot(castle_4_data, aes(x = Value, y = ..density..)) +
        ...
        geom_vline(aes(xintercept = troopValues$troop[9]), color = "blue", linewidth = 1.5)
      ...
      p5 <- ggplot(castle_5_data, aes(x = Value, y = ..density..)) +
        ...
        geom_vline(aes(xintercept = troopValues$troop[10]), color = "blue", linewidth = 1.5)
  ...
  wincheck <- reactive({
    score_calc(castle_win(troop()[1:5],troop()[6:10]))
  })

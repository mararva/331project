...
castle_win <- function(player1, player2) {
  # returns vector: "Player 1", "Player 2", or "Tie" for each castle
  out <- character(length(player1))
  for (i in seq_along(player1)) {
    if (player1[i] > player2[i]) out[i] <- "Player 1"
    else if (player1[i] < player2[i]) out[i] <- "Player 2"
    else out[i] <- "Tie"
  }
  out
}
...
score_calc_full <- function(result_vector) {
  ...
  scores <- c("Player 1" = 0, "Player 2" = 0, "Tie" = 0)
...
# Colonel samples K historical strategies and picks the best vs the player
pick_colonel_strategy <- function(player_vec, solutions_df, K = 2) {
  K <- max(1, min(K, nrow(solutions_df)))
  candidates <- sample(seq_len(nrow(solutions_df)), K, replace = FALSE)
  ...
  for (idx in candidates) {
    colonel_vec <- as.numeric(solutions_df[idx, ])
    res_vec <- castle_win(player_vec, colonel_vec)
    scores <- score_calc_full(res_vec)
    
    pscore <- as.numeric(scores["Player 1"])
    cscore <- as.numeric(scores["Player 2"])
    adv <- cscore - pscore
    
    # Primary: maximize advantage
    # Secondary: maximize Colonel score
    if (adv > best_adv || (adv == best_adv && cscore > best_cscore)) {
      best_adv <- adv
      best_cscore <- cscore
      best_idx <- idx
    }
...
server <- function(input, output, session) {
  ...
  outcome <- eventReactive(input$submit, {
    ...
    pick <- pick_colonel_strategy(pv, solutions_5, K = difficulty)
    colonel_vec <- pick$colonel_vec
    
    res_vec <- castle_win(pv, colonel_vec)
    scores  <- score_calc_full(res_vec)
    # Update cumulative record
    is_win  <- scores["Player 1"] > scores["Player 2"]
    is_loss <- scores["Player 2"] > scores["Player 1"]
  ...
  output$resultText <- renderUI({
    ...
    tagList(
      ...
      tags$p(paste0("Colonel picked historical row #", oc$picked_index,
                    " (advantage = ", oc$advantage, ")."))
    )
  })
